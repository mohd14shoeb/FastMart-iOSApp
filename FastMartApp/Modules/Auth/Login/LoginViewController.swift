...
final class LoginViewController: UIViewController {
    ...
    private func updateUI() {
        let vm = viewModel
        loginButton.isEnabled = vm.canSubmit && !vm.model.isLoading
        loginButton.alpha = vm.canSubmit ? 1.0 : 0.5
        loginButton.setTitle(vm.model.isLoading ? "Signing in..." : "Sign In", for: .normal)
        statusLabel.text = vm.model.errorMessage
        statusLabel.isHidden = vm.model.errorMessage == nil
    }
    
    @objc private func emailChanged()  {
        viewModel.update(email: emailTextField.text ?? "")
    }
    
    @objc private func passwordChanged()  {
        viewModel.update(password: passwordTextField.text ?? "")
    }
    
    @objc private func loginTapped() {
        // End editing to dismiss keyboard
